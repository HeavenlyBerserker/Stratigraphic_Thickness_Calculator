...
class StratigraphicCalculatorWindow(QMainWindow):
    ...
    def _build_tabs(self) -> None:
        ...
        average_vector_tab = self._new_model_tab(
        ...
        average_thickness_tab = self._new_model_tab(
        ...
        mixed_average_tab = self._new_model_tab(
        ...
        concentric_fold_tab = self._new_model_tab(
        ...
        plunging_fold_tab = self._new_model_tab(
    ...
    def _compute_top_normal(self, tab: ModelTab) -> None:
        ...
        mc_section = self._format_monte_carlo_section(mc_stats)
        branch_note = (
            "Top-normal = M' cos(θ+η)/cos(η) &nbsp; (S ≥ 0)"
            if result.uses_positive_s_branch
            else "Top-normal = M' cos(θ−η)/cos(η) &nbsp; (S &lt; 0)"
        )
        warn_html = self._geometry_warnings_after_result_html(result.geometry_warnings)
        output = (
            "<b>Result</b><br>"
            ...
            "<b>Quantities</b><br>"
            f"M' = {result.m_prime:.6f}<br>"
            f"θ = {result.theta_deg:.6f} deg<br>"
            f"η = {result.eta_deg:.6f} deg<br>"
            f"S = N<sub>dp</sub> . U'<sub>b</sub> = {result.s_value:.6f}<br>"
            f"Branch: {branch_note}<br>"
            "U<sub>d1</sub> (x,y,z) : "
            f"({result.ud1_vector[0]:.6f}, {result.ud1_vector[1]:.6f}, "
            f"{result.ud1_vector[2]:.6f})<br>"
            "U<sub>d2</sub> (x,y,z) : "
